...
BEGIN
    BEGIN TRY
        -- Declarar variables
        DECLARE @Id_puesto_Coordinador INT = (SELECT id_puesto FROM PUESTO WHERE nombre = 'Coordinador Rh');
        DECLARE @Id_Puesto_Empleado INT = (SELECT id_puesto FROM EMPLEADO WHERE id_empleado = @id_empleado);
        DECLARE @Id_ubicacion_empleado INT = (SELECT id_ubicacion FROM EMPLEADO WHERE id_empleado = @id_empleado);
...
BEGIN
    BEGIN TRY
        ...
        IF @Id_Puesto_Empleado = @Id_puesto_Coordinador
        BEGIN
            -- Mostrar todas las vacaciones si el empleado es Coordinador Rh
            SELECT 
                V.id_vacacion,
                E.nombre, 
            ...
            ORDER BY V.id_vacacion DESC; -- Ordenar por ID de manera descendente
        END
        ELSE
        BEGIN
            -- Mostrar solo las vacaciones de la misma ubicación si no es Coordinador Rh
            SELECT 
                V.id_vacacion,
                E.nombre, 
                CONVERT(VARCHAR(10), V.fecha_inicio, 103) AS fecha_inicio,  -- Formato dd/MM/yyyy
            ...
            LEFT JOIN EMPLEADO E ON V.id_empleado = E.id_empleado
            LEFT JOIN Estatus EST ON V.ID_Estatus = EST.ID_Estatus
            WHERE E.id_ubicacion = @Id_ubicacion_empleado
            ORDER BY V.id_vacacion DESC; -- Ordenar por ID de manera descendente
        END
    END TRY
    BEGIN CATCH
...
BEGIN
    BEGIN TRY
        ...
        IF @Id_Puesto_Empleado = @Id_puesto_Coordinador
        BEGIN
            -- Mostrar todas las incidencias si el empleado es Coordinador Rh
            SELECT I.*, 
                EST.Estatus,
                E.nombre, 
                E.apellido_paterno,
            ...
            ORDER BY I.id_incidencia DESC; -- Ordenar por ID de manera descendente
        END
        ELSE
        BEGIN
            -- Mostrar solo las incidencias de empleados en la misma ubicación si no es Coordinador Rh
            SELECT I.*, 
                EST.Estatus,
                E.nombre, 
                E.apellido_paterno,
...
BEGIN
    BEGIN TRY
        ...
        IF @Id_Puesto_Empleado = @Id_puesto_Coordinador
        BEGIN
            -- Mostrar todas las horas extras si el empleado es Coordinador Rh
            SELECT HE.*, 
                EST.Estatus, 
                E.nombre, 
                E.apellido_paterno,
            ...
            ORDER BY HE.id_horaExtra DESC; -- Ordenar por ID de manera descendente
        END
        ELSE
        BEGIN
            -- Mostrar solo las horas extras de la misma ubicación si no es Coordinador Rh
            SELECT HE.*, 
                EST.Estatus, 
                E.nombre, 
                E.apellido_paterno,
...
BEGIN
    BEGIN TRY
        ...
        IF @Id_Puesto_Empleado = @Id_puesto_Coordinador
        BEGIN
            -- Mostrar todos los préstamos si el empleado es Coordinador Rh
            SELECT 
                P.id_prestamo,
                P.folio_prestamo,
                P.hora_registro,
            ...
            ORDER BY P.id_prestamo DESC; -- Ordenar por ID de manera descendente
        END
        ELSE
        BEGIN
            -- Mostrar solo los préstamos de la misma ubicación del empleado
            SELECT 
                P.id_prestamo,
                P.folio_prestamo,
                P.hora_registro,
...
BEGIN
    BEGIN TRY
        ...
        IF @Id_Puesto_Empleado = @Id_puesto_Coordinador
        BEGIN
            -- Mostrar todos los justificantes si el empleado es Coordinador Rh
            SELECT 
                J.id_justificante, 
                J.fecha_registro, 
            ...
            INNER JOIN PUESTO P ON E.id_puesto = P.id_puesto
            ORDER BY J.id_justificante DESC; -- Ordenar por ID de manera descendente
        END
        ELSE
        BEGIN
            -- Mostrar justificantes filtrados por la ubicación del empleado
            SELECT 
                J.id_justificante, 
                J.fecha_registro, 
                J.Permiso_solicitado,
...
BEGIN    
    ...
    SELECT     
        ...
        -- Nueva columna: TieneEvaluacion    
        CASE    
            WHEN EXISTS (    
                SELECT 1    
                FROM ORDEN_MTTO O    
                INNER JOIN CIERRE_ORDEN CO ON O.id_orden = CO.id_orden    
                WHERE O.id_solicitud = SMT.id_solicitud    
            ) THEN 1    
            ELSE 0    
        END AS TieneEvaluacion    
    ...
    ORDER BY SMT.id_solicitud DESC;    
END;    
GO
...
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'SP_Obtener_Menus_ConEstado')
BEGIN
...
GO
 ...
 END AS EstaAsignado     FROM Menu M     LEFT JOIN Menu_Usuario MU ON MU.ID_Menu = M.ID_Menu AND MU.ID_Usuario = @IDASP; 
 END 
 GO
